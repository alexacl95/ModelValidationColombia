function sol = ChimeraExtension(params,time,ins)
    domain = time(2);         % Simulation domain
    M = zeros(23, domain + 1);  % Simulation matriz
    
  
    %Parameters
    beta_L = params(1,:);       % beta_L
    beta_T = params(2,:);      % beta_T 
    beta_P = params(3,:);      % beta_P
    phi_EP = params(4,:);      % phi_{EP}
    lambda_fq = params(5,:);   % lambda_fq_{fq}
    vartheta_E = params(6,:);  % varvartheta_E_E
    gamma_L = params(7,:);     % gamma_L_L
    k_L  = params(8,:);        % k_L
    k_P  = params(9,:);        % k_P
    phi_T   = params(10,:);     % phi_T
    lambda_qf = params(11,:);   % lambda_{qf}
    psi_e = params(12,:);       % psi_e
    phi_PH = params(13,:);      % phi_{PH}
    delta = params(14,:);       % delta
    
    %Inputs
    m = params(15,:);           %Number of inmigrants
    
    %Modifying parameters
    eta_L = params(16,:);       % eta_L
    vartheta_P = params(17,:);  % vartheta_P
    eta = params(18,:);         % gamma_H
    z = params(19,:);           % z
    phi_PL = params(20,:);      % phi_{PL}
    eta_vartheta = params(21,:);% eta_vartheta
    %nons = params(22,:);        % initial proportion of inmune population
    ...
    b_H = params(29,:);         % b_H
    nu = params(30,:);          % nu
    lon = round(params(31,:));  % time
    time(1)=round(params(32,:));%breakpoint
    fixing = params(33,:); %recálculo de S
    
    M(:,1:time(1))=ins.M(:,1:time(1));
    
    correction = min(fixing*M(1,1),M(18,time(1)));
    M(1,time(1)) = M(1,time(1)) + correction;
    M(18,time(1)) = M(18,time(1)) - correction;
    ...
    L(1:time(1),:)=ins.L(1:time(1),:);
    JL(1:time(1),:)=ins.JL(1:time(1),:);
    JH(1:time(1),:)=ins.JH(1:time(1),:);
    k_L = k_P * k_L;
    ...
    if ins.action==2
        ...
        if sum(M(1 : 2, end)) < M(1, 1) * 0.1
            pr(end, :) = [0 0.25];
            nom(end,:) = (pr(end, 1) + pr(end, 2))/2;
            if ~strcmp(ins.rnames{end},'reg')
